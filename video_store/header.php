<html>

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    
    <meta name="viewport" content="width=device-width, initial-scale=1"> <!--  Help to scale to fit screen either large medium or small    -->

      <title> Video Rental</title>
    <script src="js/respond.js"></script>
    <link rel ="stylesheet" href="css/bootstrap.min.css">
    <script src="//tinymce.cachefly.net/4.1/tinymce.min.js"></script>

   <!-- <script>
            tinymce.init({selector:'textarea'});
     </script>
     -->
    <link rel ="stylesheet" href="css/custom_style.css">
    <link rel ="stylesheet" href="css/project_style.css">

</head>


<body class="bdy">

<div class="wrap">

  <nav class="navbar navbar-inverse navbar-fixed-top">
     <div class="container-fluid">
       <a href="#" class=" navbar-brand"><img src="/video_store/image/vid_reel.jpg"> 
         <h2> Oblack Video  Rental </h2></a> 
     
        <a href="#"> <div class="flo"><img src="/video_store/image/vid_reel.jpg"> 
        </div>
        </a>  
    </div>
  </nav> 

  <!--Preparing the main page-->
 
      <div class="container-fluid">
         <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12" style="background:;height:400px;">
             <br><br><br>
        
              <div class = "page-header " >
                <span class="glyphicon glyphicon-film gi-4x"></span>
               <b>Welcome to Oblak Video Store</b>
              </div>
             
        

  
  <div class="col-sm-3 col-xs-3 col-md-3 bg-info sidebar " >
 
   <ul class="nav nav-pills">
     <li><a href="check_in_out.php" >Check-In / Check-Out</a></li><br><br>
      <li><a href="add_cust.php">Sign-Up</a></li><br><br>
       <li><a href="search_title.php">Search </a></li><br><br>
        <li><a href="add_film.php">Add Film Title</a></li><br><br>
        <li><a href="add_copy.php">Add Film Copy</a></li><br><br>
         <li><a href="add_rent.php">Rent</a></li><br><br>
        <li><a href="add_artist.php">Add Artist</a></li><br><br>
         <li><a href="add_return.php">Return</a></li><br><br>
          <li><a href="log_out.php">Log-out</a></li>
    </ul>
  </div>
   